...
// Plugin constants.
define( 'LW_SITE_MANAGER_VERSION', '1.1.27' );
...
} elseif ( ! class_exists( \LightweightPlugins\SiteManager\Plugin::class ) ) {
	add_action(
		'admin_notices',
...
final class Plugin {
	...
	private function register_woocommerce_categories(): void {
		...
		wp_register_ability_category(
			'wc-attributes',
			[
				'label'       => __( 'WooCommerce Attributes', 'lw-site-manager' ),
				'description' => __( 'Global attributes, attribute terms, product-attribute bindings, and variations', 'lw-site-manager' ),
			]
		);
	}
